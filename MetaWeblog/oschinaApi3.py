...
def _http_request(url, method, authorization, **kw):
    ...
    req = urllib.request.Request(http_url, data=http_para, headers=headers)
    if authorization:
...
class APIClient(object):
    def __init__(self, app_key, app_secret, redirect_uri=None, response_type='code'):
        ...
        self.auth_url = 'https://www.oschina.net/action/openapi/token'
        self.api_url = 'http://www.oschina.net'
    ...
    def get_authorize_url(self):
        return "https://www.oschina.net/action/oauth2/authorize?response_type=code&client_id=%s&redirect_uri=%s" % (
        self.client_id, self.redirect_uri)

    # post a request and then get a access_token
    def request_access_token(self, code):
        result = _http_post(self.auth_url,
                            client_id=self.client_id,
                            client_secret=self.client_secret,
                            redirect_uri=self.redirect_uri,
                            code=code, grant_type='authorization_code', dataType='json')
    ...
    # 发帖
    def post_pub(self):
        result = _http_post('%s%s' % (self.api_url, '/action/openapi/post_pub'),
