...
class OsChina(object):
    ...
    def post(main, timeout):
        ...
        # 5.点击弹出右侧窗口
        my_space = WebDriverWait(driver, timeout).until(
            lambda d: d.find_element_by_xpath('/html/body/div[2]/div/div[3]/a[2]'))
        my_space.click()
        #6.点击"写博客"
        write_blog = WebDriverWait(driver, timeout).until(
                lambda d: d.find_element_by_xpath('//*[@id="userSidebar"]/a[3]'))
        write_blog.click()
        # 7.选择自定义分类, 系统分类
        ...
        # 8.填写标题, 内容和标签
        title = driver.find_element_by_xpath('//*[@id="writeArticleWrapper"]/div/div/form/div[1]/div[1]/input')
        title.clear()
        title.send_keys(main.title)
        content = driver.find_element_by_xpath('//*[@id="editormd"]')
        # content.clear()
        content.send_keys(main.content)
        tags = driver.find_element_by_xpath('//*[@id="writeArticleWrapper"]/div/div/form/div[4]/div[1]/div')
        tags.clear()
